...
async function runMigration() {
  try {
    ...
    // Read migration SQL
    const migrationPath = path.join(__dirname, 'prisma', 'migrations', '20260713120000_add_saving_transactions', 'migration.sql');
      ...
      if (statement) {
        try {
          ...
          if (statement.includes('CREATE TABLE')) {
            const tableName = statement.match(/CREATE TABLE "(\w+)"/)?.[1] || 'table';
          ...
          if (error.message.includes('already exists')) {
            ...
            if (statement.includes('CREATE TABLE')) {
              ...
              const tableName = statement.match(/ALTER TABLE "(\w+)"/)?.[1] || 'table';
              console.log(`   ⚠️  Column already exists in ${tableName}, skipping...`);
            } else {
